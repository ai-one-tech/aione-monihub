use aione_monihub_server::shared::{generate_snowflake_id, validate_snowflake_id, extract_timestamp_from_id};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(timestamp) = extract_timestamp_from_id(&id1) {
        let datetime = chrono::DateTime::from_timestamp_millis(timestamp as i64)
            .unwrap_or_default();
        println!("   提取的时间戳: {} ({})", timestamp, datetime.format("%Y-%m-%d %H:%M:%S%.3f UTC"));
    }
    println!();
    ...
    println!("   有效ID验证:");
    for id in &valid_ids {
        println!("     {} -> {}", id, if validate_snowflake_id(id) { "有效" } else { "无效" });
    }
