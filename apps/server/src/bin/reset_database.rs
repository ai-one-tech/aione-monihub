use sea_orm::{Database, DatabaseConnection, DbErr, Statement, FromQueryResult};
...
async fn execute_migrations(pg_pool: &PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for entry in migration_files {
        ...
        execute_sql_file(pg_pool, &sql_content).await
...
async fn execute_sql_file(pg_pool: &PgPool, sql_content: &str) -> Result<(), Box<dyn std::error::Error>> {
