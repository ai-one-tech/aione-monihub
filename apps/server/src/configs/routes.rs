...
pub fn config_routes(cfg: &mut web::ServiceConfig) {
    cfg.route("/api/configs", web::get().to(handlers::get_configs))
        .route("/api/configs", web::post().to(handlers::create_config))
        .route(
            "/api/configs/code/{code}",
            web::get().to(handlers::get_config_by_code),
        )
        .route(
            "/api/configs/code/{code}/environment/{environment}",
            web::get().to(handlers::get_config_by_code_and_environment),
        )
        .route(
            "/api/configs/code/{code}/environment/{environment}/version/{version}",
