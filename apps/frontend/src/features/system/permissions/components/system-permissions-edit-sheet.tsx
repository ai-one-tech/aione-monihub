...
} from '../hooks/use-permission-mutations'
...
export function SystemPermissionsEditSheet() {
  ...
  const {
    ...
    error,
  } = usePermissionDetailQuery(selectedPermissionId)
  ...
  useEffect(() => {
    if ((isEditMode || isViewMode) && permissionDetail) {
      ...
      Promise.resolve().then(() => {
        ...
        form.setValue(
          'parent_permission_id',
          permissionDetail.parent_permission_id || ''
        )
      ...
      updatePermissionMutation.mutate(
        {
          ...
          permissionData: submitData as UpdatePermissionRequest,
        },
        {
          onSuccess: () => {
            setIsPermissionSheetOpen(false)
  ...
  const selectedParent = allPermissions.find(
    (p) => p.id === form.watch('parent_permission_id')
  )
                      ...
                      <FormItem>
                        ...
                        <Popover open={parentOpen} onOpenChange={setParentOpen}>
                          <PopoverTrigger asChild>
                            ...
                            >
                              {selectedParent
                                ? `${selectedParent.name}${selectedParent.description ? `（${selectedParent.description}）` : ''}`
                                : '请选择父级权限'}
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        >
                          ...
                          <SelectContent>
                            {Object.entries(PERMISSION_TYPE_MAP).map(
                              ([key, value]) => (
