  ...
  if (!task || !results) {
    return (
      <div className='flex flex-col items-center justify-center py-12'>
        ...
        <p className='text-muted-foreground text-sm'>无法加载任务信息</p>
      </div>
    )
  }
...
function StatusCard({
  ...
  return (
    ...
    >
      ...
      />
      <div className='mb-0.5 text-2xl font-bold'>{count}</div>
    ...
    <div className='hover:bg-accent/50 rounded-lg border p-4 transition-colors'>
      <div className='mb-3 flex items-start justify-between'>
        <div className='flex items-center gap-2'>
          ...
          />
          <div>
            <div className='text-sm font-medium'>{result.instance_id}</div>
      ...
      {result.status === 'running' && result.started_at && (
        <div className='mb-3'>
          <div className='text-muted-foreground mb-1 text-xs'>执行时间</div>
      ...
      {result.status === 'success' && (
        <div className='mb-3 space-y-2'>
          ...
          {result.result_data && (
            <div>
              <div className='text-muted-foreground text-xs'>执行结果</div>
              <div className='bg-muted mt-1 max-h-20 overflow-hidden rounded p-2 font-mono text-sm'>
      ...
      {result.status === 'failed' && result.error_message && (
        <div className='mb-3'>
          <div className='text-muted-foreground mb-1 text-xs'>错误信息</div>
